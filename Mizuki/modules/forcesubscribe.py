...
from Mizuki import tbot, BOT_ID
...
async def fs(event):
  ...
  if args in ("off", "no", "disable"):
    ...
    if rip is False:
      return await event.reply(f"❗**Not an Admin in the Channel**\nI am not an admin in the [channel](https://t.me/{args}). Add me as a admin in order to enable Force Subscribe.", link_preview=False)
