from Mizuki.modules.sql.nightmode_sql import add_nightmode, rmnightmode, get_all_chat_id, is_nightmode_indb
from telethon.tl.types import ChatBannedRights
from apscheduler.schedulers.asyncio import AsyncIOScheduler 
from telethon import functions
from Mizuki.events import register
from Mizuki import tbot, OWNER_ID
import os
    ...
    if not event.sender_id == OWNER_ID:
        if not await is_register_admin(event.input_chat, event.sender_id):
          ...
          if not await can_change_info(message=dmod):
            await event.reply("You are missing the following rights to use this command > Can Changeinfo.")
            return
    if not input:
